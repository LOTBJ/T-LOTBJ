# -*- coding: utf-8 -*-
"""
DSH 鲸鱼娘桌宠 —— bigfish(dafeiyu-pet) 基底 + DSH 集成提升
依附启动器: 由 dsh-launcher.ps1 拉起/退出
提升: DSH 会话联动(忙→思考/完成→庆祝) + DSH 控制菜单(打开/重启/装配)
...
class PetWindow(QWidget):
    ...
    def check_system_status(self):
        """DSH 会话联动(提升点): ~/.dsh/sessions 15s 内有写入=主人在忙
        忙→思考动作+台词; 忙完(持续>20s)→跳跃庆祝"""
        now = self.t * TICK
        if now - getattr(self, "last_dsh_check", 0) < 3000:
            return
        self.last_dsh_check = now
        busy = False
        try:
            sdir = os.path.join(os.path.expanduser("~"), ".dsh", "sessions")
            ...
            pass
        if busy and not getattr(self, "dsh_busy", False):
            self.dsh_busy = True
            self.dsh_busy_at = time.time()
            self.say("主人在忙呢，我陪你一起~")
        elif not busy and getattr(self, "dsh_busy", False):
            self.dsh_busy = False
            if getattr(self, "dsh_busy_at", 0) and time.time() - self.dsh_busy_at > 20:
                self.jump_t = 1.0
                self.say("忙完啦！任务完成，辛苦啦~")

    # ---------- DSH 控制(提升点): 打开/重启/装配 ----------
    def _dsh_url(self):
        return "http://127.0.0.1:3080"

    def _dsh_running(self):
        import socket
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(0.5)
        try:
            s.connect(("127.0.0.1", 3080))
            s.close()
            return True
        except Exception:
            return False
    ...
    def _open_dsh(self):
        def worker():
            try:
                if not self._dsh_running():
                    self._queue_say("服务没开，我先帮你启动~")
                    self._start_dsh_server()
                    for _ in range(40):
                        if self._dsh_running():
                            break
                        time.sleep(1)
                # PWA 优先(鲸鱼娘图标), 否则默认浏览器
                try:
                    subprocess.Popen(
                        ["powershell", "-NoProfile", "-Command",
                         'explorer.exe "shell:AppsFolder\\MSEdge._crx__hgiemfgfjboikeiepnnjapnpc"'],
                        creationflags=subprocess.CREATE_NO_WINDOW)
                except Exception:
                    os.startfile(self._dsh_url())
                self._queue_say("打开啦，去 DSH 里玩吧~")
            except Exception as e:
                self._queue_say("打开失败: " + str(e)[:20])
        threading.Thread(target=worker, daemon=True).start()

    def _start_dsh_server(self):
        home = os.path.expanduser("~")
        dsh_bin = os.path.join(home, "AppData", "Local", "npm-cache", "_npx",
                               "1e7f6d9597241db0", "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js")
        if not os.path.exists(dsh_bin):
            raise Exception("dsh not found")
        node = r"C:\Program Files\nodejs\node.exe"
        if not os.path.exists(node):
            node = "node"
        subprocess.Popen([node, dsh_bin, "web"],
                         cwd=os.path.dirname(os.path.abspath(__file__)),
                         creationflags=subprocess.CREATE_NO_WINDOW)

    def _restart_dsh(self):
        def worker():
            self._queue_say("收到，正在重启 DSH~")
            try:
                subprocess.run(
                    ["powershell", "-NoProfile", "-Command",
                     "Get-NetTCPConnection -LocalPort 3080 -State Listen | Select-Object -First 1 | "
                ...
                for _ in range(45):
                    if self._dsh_running():
                        break
                    time.sleep(1)
                self._queue_say("重启好啦！")
                self._open_dsh()
            except Exception as e:
                self._queue_say("重启失败: " + str(e)[:20])
        threading.Thread(target=worker, daemon=True).start()

    def _setup_dsh(self):
        def worker():
            self._queue_say("检测环境中…")
            try:
                import shutil
                if not shutil.which("node"):
                    self._queue_say("没装 Node.js，去 https://nodejs.org 装一下~")
                    return
                if shutil.which("dsh"):
                    self._queue_say("DSH 已经装好啦~")
                    return
                self._queue_say("开始装配 DSH（约 1-2 分钟）…")
                subprocess.run(
                    ["cmd", "/c", "npm install -g @deepseek-ai/dsh --registry=https://registry.npmmirror.com"],
                    creationflags=subprocess.CREATE_NO_WINDOW, capture_output=True)
                if shutil.which("dsh"):
                    self._queue_say("装配完成！随时可以开工~")
                else:
                    self._queue_say("装配没成功，看看日志再试~")
            except Exception as e:
                self._queue_say("装配失败: " + str(e)[:20])
        threading.Thread(target=worker, daemon=True).start()
    ...
    def _build_menu(self):
        ...
        m.addSeparator()
        # DSH 控制(提升点)
        dsh_menu = m.addMenu("🔧 DSH 控制")
        dsh_menu.addAction("▶ 打开 DSH", self._open_dsh)
        dsh_menu.addAction("↻ 重启 DSH", self._restart_dsh)
        dsh_menu.addAction("🔧 装配 DSH 环境", self._setup_dsh)
        m.addSeparator()
