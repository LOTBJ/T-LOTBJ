...
DeepSeek Harness（DSH）的 Windows 桌面启动器：一键打开/重启本地服务，带极简控制台与鲸鱼娘桌宠两个主题。
...
- **极简模式**（默认）：DeepSeek 官网风格 —— 暖白底 + 顶部淡蓝晕染（"纱布撒蓝"）、大字距标题、玻璃胶囊按钮、footer 小字链接
- **鲸鱼娘模式**：深海渐变舞台 + 标准 Codex 图集桌宠（8×11，192×208/格）
  - 动画：待机 / 左右行走 / 挥手 / 跳跃 / 失败 / 等待 / 思考 / 检查 / 16 方向注视
  - 互动：**左键拖动换位置（侧身）**、**双击跳跃**、**右键菜单**（散步 / 跟随鼠标 / 原地待着 / 喂食 / 缩放 / 退出）
...
- **五层防火墙**：配置快照、坏条目隔离、安全模式、自动降级、UI 异常兜底（事件处理器异常不再崩进程）
...
| 参数 | 说明 |
|---|---|
| `-Theme whale` | 启动时直接进入鲸鱼娘模式（默认 minimal） |
| `-Theme minimal` | 极简模式 |
